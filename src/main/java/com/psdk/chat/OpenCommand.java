...
public class OpenCommand implements CommandExecutor {
    ...
    public OpenCommand(PSDK plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("psdk.openchat")) {
            sender.sendMessage(mm.deserialize("<#FF0000>Você não tem permissão!"));
            return true;
        }

        if (plugin.getChatManager().isChatEnabled()) {
            sender.sendMessage(mm.deserialize("<#00FF00>O chat já está aberto!"));
            return true;
        }
        ...
        Bukkit.broadcast(mm.deserialize("<#00FF00>O chat foi aberto por:<reset> " + adminDisplay));
        return true;
    }
}
