...
public class ResetCommand implements CommandExecutor, TabCompleter {
    ...
    public ResetCommand(PSDK plugin) { this.plugin = plugin; }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage(MM.deserialize("<#fcc850>Uso: /resetplayer <jogador>"));
            return true;
        }
        ...
        plugin.getPlayerDataManager().resetPlayerData(uuid);
        sender.sendMessage(MM.deserialize(
                "<#10fc46>Dados de <#cbd1d7>" + args[0] + " <#10fc46>resetados completamente!"));

        Player online = Bukkit.getPlayer(uuid);
        if (online != null)
            online.sendMessage(MM.deserialize("<#e22c27>Seus dados foram resetados por um administrador."));
        return true;
    }
