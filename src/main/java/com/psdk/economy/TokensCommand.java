...
public class TokensCommand implements CommandExecutor, TabCompleter {
    ...
    public TokensCommand(PSDK plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        double bal = plugin.getEconomyManager().getTokens(target.getUniqueId());

        if (target == sender) {
            sender.sendMessage(mm.deserialize("<#fcc850>Seus TOKENS: <#10fc46>" + com.psdk.util.NumberUtil.abbrev(bal)));
        } else {
            sender.sendMessage(mm.deserialize("<#fcc850>TOKENS de <#efa600>" + target.getName() +
